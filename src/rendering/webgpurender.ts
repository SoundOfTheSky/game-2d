...
export class WebGPURenderer {
  ...
  private _UBOs = new Compiled<
    Mesh,
    { data: Float32Array; buffer: GPUBuffer }
  >()
  private _pipelines = new Compiled<
    Mesh,
  ...
  private _createBuffer(
    ...
    new (data.constructor as Float32ArrayConstructor)(
    ...
    if (!target || texture.needsUpdate) {
      ...
      if (texture.image instanceof HTMLVideoElement) {
        ...
        target = this.device.createTexture({
          format:
            (texture.format as GPUTextureFormat | undefined) ?? this.format,
          dimension: '2d',
          size: [width, height, 1],
          usage:
  ...
  compile(mesh: Mesh, camera?: Camera): void {
    ...
    for (const key in mesh.geometry.attributes) {
      ...
      const formatType =
        attribute.data instanceof Float32Array ? 'float' : 'uint'
      ...
      buffers.push({
        arrayStride:
          attribute.size *
          attribute.data.BYTES_PER_ELEMENT *
          (attribute.divisor || 1),
        ...
        attributes: [
          {
            ...
            offset: 0,
            format:
              `${formatName}x${Math.min(attribute.size, 4)}` as GPUVertexFormat,
          },
    ...
    const depthCompare = (
      mesh.material.depthTest ? 'less' : 'always'
    ...
    if (pipeline?.label !== pipelineCacheKey) {
    ...
    for (const key in mesh.geometry.attributes) {
      ...
      if (!buffer) {
        buffer = this._createBuffer(
          attribute.data,
          isIndex ? GPU_BUFFER_USAGE_INDEX : GPU_BUFFER_USAGE_VERTEX,
        )
      ...
      if (this._passEncoder instanceof GPURenderPassEncoder) {
        if (isIndex)
          this._passEncoder.setIndexBuffer(
            buffer,
            `uint${attribute.data.BYTES_PER_ELEMENT * 8}` as GPUIndexFormat,
          )
    ...
    const parsed = parseUniforms(
      mesh.material.vertex,
      mesh.material.fragment,
      mesh.material.compute,
    )
    if (parsed) {
      ...
      if (!UBO) {
        ...
        this._UBOs.set(
          mesh.material as unknown as Mesh & { dispose: () => void },
          UBO,
          () => {
    ...
    for (const key in mesh.material.uniforms) {
      ...
      if (value instanceof Texture) {
        ...
        entries.push({
          binding: binding++,
          resource:
            target instanceof GPUTexture ? target.createView?.() : target,
        })
      }
    }
   ...
   */
  public render(scene: Object3D, camera?: Camera): void {
    ...
    this._passEncoder = this._commandEncoder.beginRenderPass({
      colorAttachments: renderViews.map<GPURenderPassColorAttachment>(
        (view) => ({
          view,
          resolveTarget,
          loadOp,
    ...
    for (const node of renderList) {
      ...
      if (index)
        this._passEncoder.drawIndexed(
          min(count, index.data.length / index.size),
          node.instances,
          start,
        )
      else if (position)
        this._passEncoder.draw(
