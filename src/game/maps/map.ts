import { default as createPlayer } from '../entities/player'
...
export default class Map {
  ...
  public constructor(
    ...
    const map = JSON.parse(this.world.resources[`/game/maps/${name}.json`] as string) as TiledMap
    map.tilesets = map.tilesets.map((tileset) => {
      tileset.sourceFile = this.world.resources[`/game/tilesets/${tileset.source.slice(12, -5)}.png`] as HTMLImageElement
      tileset.columns = tileset.sourceFile.naturalWidth / map.tilewidth
      return tileset
    })
    this.mapSystem.tiledDefinition = map

    const spawn = (map.layers.find(layer => layer.name === 'triggers') as TyledObjectLayer).objects.find(
